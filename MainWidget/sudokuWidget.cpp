    ...
    : QWidget(parent)
{
    ...
    connect(&udpReceiver, SIGNAL(readyRead()),this, SLOT(processPendingDatagrams()));
    ...
    connect(cB_theme,SIGNAL(currentIndexChanged(const QString &)),this,SLOT(changeTheme(const QString &)));
    connect(cB_background,SIGNAL(currentIndexChanged(const QString &)),this,SLOT(changeBackground(const QString &)));
