...
int sudokuMain::GetSolution(int table[9][9][10],int solution[9][9])
{
   ...
   //Check Frist To Kill Morbid Table
    ...
    for(k=0;k<9;k++)
    {
        for(i=list[k]/10,index = 0;i<(list[k]/10)+3;i++)//The Nine "33" 's Square Each Number Should Be Unique
        {
    ...
    do
    {
        ...
        if(bStepBack == 0)
        {
            ...
            for(k=0;k<10;k++){table[T_L][T_R][k] = 0;}//All To "0"

            for(j=0;j<9;j++)//The Same Row With Position Of [T_L][T_R]
            {
                tempT = table[T_L][j][0];
                if(j != T_R){table[T_L][T_R][tempT] = tempT;}
            }

            for(i=0;i<9;i++)//The Same Column With Position Of [T_L][T_R]
            {
                tempT = table[i][T_R][0];
                if(i != T_L){table[T_L][T_R][tempT] = tempT;}
            }
            ...
            {
                if(table[T_L][T_R][k] == 0){table[T_L][T_R][k] = k; bGotValuable = 1;}
                else{table[T_L][T_R][k] = 0;}
            }
        }
        else
        {
            ...
            {
                if(table[T_L][T_R][k] != 0){bGotValuable = 1;break;}
            }
        }
